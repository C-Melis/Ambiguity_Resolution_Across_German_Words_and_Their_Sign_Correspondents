...
**Sense and Sign ID Files**: Contains meaning-video_id mappings, where the meaning column combines German translations with sense clarifications for each sign. These files provide the semantic descriptions that enable similarity-based matching (also, lexical matching initially for EM, although the ranking uses similarity again). An additional variant containing only German translations (without sense clarifications) is used for ablation testing.
